...
@section('content')
                    ...
                    <form method="GET" action="{{ route('laporan.bulanan') }}">
                        @if ($bulan && $tahun)
                            <div class="alert alert-info">
                                ...
                                <strong>{{ \Carbon\Carbon::createFromDate($tahun, $bulan)->translatedFormat('F Y') }}</strong>
                            </div>
                        @endif
                ...
                <div class="card-header d-flex align-items-start pb-0 cursor-pointer"
                    ...
                    <div>
                        <h2 class="text-bold-700 mb-0">{{ number_format($jumlahKg, 2, ',', '.') }} Kg
                            ({{ $laporanKgCustomer->where('bulan', $bulan)->sum('total_pcs') }}
                            Pcs)</h2>
                        <p>Laundry Reguler {{ \Carbon\Carbon::create()->month($bulan)->translatedFormat('F') }}
                            {{ $tahun }}</p>
                    </div>
                </div>
                ...
                <div class="card-header d-flex align-items-start pb-0 cursor-pointer"
                    ...
                    <div>
                        <h2 class="text-bold-700 mb-0">
                            {{ number_format($laporanSatuanCustomer->where('bulan', $bulan)->sum('total_pcs'), 2, ',', '.') }}
                            Pcs</h2>
                        <p>Laundry Satuan
        ...
        <div class="col">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <h4 class="card-title">Grafik Laundry Harian</h4>
                    <span>{{ \Carbon\Carbon::createFromDate($tahun, $bulan)->translatedFormat('F Y') }}</span>
                </div>
                <div class="card-content">
                    <div class="card-body pb-0">
                        <div style="overflow-x: auto;">
                            <div id="grafik-kg-pcs-harian" style="min-width: 1500px;"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @php
        ...
        // Group data KG dan PCS
        $jenisKg = $kgPerHariPerJenis->groupBy('jenis_grouped');
        $jenisPcs = $pcsPerHariPerJenis->groupBy('jenis_grouped');

        // Buat data untuk grafik KG
        $_kg_series = [];
        foreach ($jenisKg as $jenis => $records) {
            $data = [];
            foreach ($tanggalList as $tanggal) {
                $found = $records->firstWhere('tanggal', $tanggal);
                $data[] = $found ? (float) $found->total_kg : 0;
            }
            $_kg_series[] = ['name' => $jenis, 'data' => $data];
        }

        // Buat data untuk grafik PCS
        $_pcs_series = [];
        foreach ($jenisPcs as $jenis => $records) {
            $data = [];
            foreach ($tanggalList as $tanggal) {
                $found = $records->firstWhere('tanggal', $tanggal);
                $data[] = $found ? (int) $found->total_pcs : 0;
            }
            $_pcs_series[] = ['name' => $jenis, 'data' => $data];
        }
    @endphp
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var tanggalList = {!! json_encode($tanggalList) !!};
        var kgSeries = {!! json_encode($_kg_series) !!};
        var pcsSeries = {!! json_encode($_pcs_series) !!};

        // Gabungkan series KG dan PCS, beri yAxisIndex
        var combinedSeries = [];
        ...
        var combinedChart = new ApexCharts(
            document.querySelector("#grafik-kg-pcs-harian"),
